...
def test_fill_in_form():
    ...
    browser.all('[for^=hobbies').element_by(have.exact_text('Reading')).perform(command.js.scroll_into_view)
    ...
    browser.element('#uploadPicture').set_value(
        os.path.abspath(
            os.path.join(os.path.dirname(tests.__file__), 'resources/foto.png')
        )
    )
    '''
    ...
    # # Вариант 1: Прокликиваем дату в календаре
    # browser.all(".react-datepicker__year-select").element_by(have.exact_text("2002")).click()
    ...
    # browser.all(".react-datepicker__year-select").type('2002')
    # browser.all(".react-datepicker__week").all("")
    #
    # # Вариант 2: Устанавливаем сразу
    # browser.element("#firstName").type("Misha")